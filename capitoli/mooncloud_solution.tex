...
L'obiettivo del progetto Moon Cloud è quello di implementare una metodologia automatica per la valutazione della sicurezza, delle performance e di altre proprietà non funzionali, fornendo un processo di assurance basato su attività di auditing e raccolta di evidenze.
...
L'obiettivo di questo capitolo è quello di dare al lettore il \texit{know-how} necessario a comprendere le motivazioni legate ad alcune decisioni prese durante la realizzazione del lavoro di tesi, in particolar modo per gestire alcune limitazioni del framework Moon Cloud.
...
\begin{itemize}
    \item \textbf{Metriche}: insieme di proprietà non funzionali delle quali si vogliono ottenere \textbf{misurazioni}.
    ...
    \item \textbf{Regola di valutazione astratta}, o \textit{AER (abstract evaluation rule)}, regola logica che implementa un processo di valutazione. È data dall'aggregazione di controlli mediante operatori \textit{booleani}. I suoi termini possono essere sia \textit{controlli} che altre \textit{AER}.
    \item \textbf{Regola di valutazione concreta}, o \textit{ER (evaluation rule)}: istanza di una \textit{AER}. I suoi termini possono essere \textit{test} (mappati sui rispettivi \textit{controlli}, oppure altre \textit{ER}.
\end{itemize}
\vfill
\newpage
...
\end{figure}
L'architettura del framework Moon Cloud e l'interazione tra i vari microservizi che lo compongono sono illustrate in figura \ref{fig:MCArchi}.
...
\begin{itemize}
    ...
    \item \textbf{API} (3), costituiscono l'intefaccia con le principali funzionalità del sistema. Interagiscono con un database di management (4) seguendo il paradigma RESTful, e orchestrano l'esecuzione dei test.
        ...
        \begin{itemize}
            \item in modalità one-shot, inviando un messaggio agli \textit{execution manager} (9) tramite un meccanismo di comunicazione basato su code(5).
...
\begin{itemize}
    ...
    \item \textbf{Execution Manager} (10), ovvero il servizio che gestisce il lavoro di ciascun nodo del cluster.        
        Gli \textit{execution manager} di uno stesso cluster sono connessi alla stessa coda: all'arrivo di un messaggio essi ne effettuano il \textit{prefetch}, ne validano il contenuto, effettuano il download dal repository del driver relativo al controllo referenziato dal test (11), eseguono lo stesso con gli input contenuti nel messaggio; infine collezionano gli output del test in un database non relazionale \textit{time series}(12). % ELIA: nell'immagine vi sono due frecce col 10 e nessuna con l'11
...
\begin{definition}[\textbf{Recommendation verification}]\label{def:prop}
    Sia $\Rec{}$ la \textit{recommendation} da verificare, la \textit{recommendation verification} è una funzione  $\widehat{\Rec{}}$  definita sulla tupla $\langle$$\EP{}$, $\ER{}$$\rangle$ che restituisce un valore booleano \{true, false\} dove: % ELIA: sembra i simboli non sono rappresentati bene in output nel pdf)
...
\begin{itemize}
    \item $\EP{}$ è l'insieme di \textit{processi di valutazione} \{$\Eval{}$\} da eseguire. L'\textit{output} del processo di valutazione è un valore \textit{booleano} che esprime se la valutazione abbia avuto successo o no, aggregato alle \textit{evidence} collezionate a supporto del risultato.
        \item $\ER{}$ è una \textit{evaluation rule} espressa come formula di logica proposizionale, i cui termini sono i singoli processi di valutazione $\Eval{}$. Essa combina il risultato di vari processi $\Eval{}$$\in$$\EP{}$ e restituisce \textit{true} se la valutazione ha successo, \textit{false} altrimenti. % ELIA occhio ai simboli a inizio riga
\end{itemize}
\end{definition}
...
Un controllo $C$ specifica i dettagli su come collezionare le evidenze su un target $t$ per valutare la \textit{recommendation}. È definito nel seguente modo:
...
Moon Cloud è in grado di eseguire molteplici processi di valutazione $ \Eval{} $ in parallelo, ciascuno dei quali si riferisce ad un insieme di raccomandazioni $\Rec{}$ che devono essere valutati.
...
\begin{itemize}
    ...
    \item TOKEN\_IFF $\rightarrow$ iff(term1, term2), operatore binario che implementa l'operatore di implicazione doppia "se e solo se": $return ~ (\sim term1 \vee term2) \wedge (\sim term2 \vee term1)$
\end{itemize}
\textit{TOKEN\_LPAREN} e \textit{TOKEN\_RPAREN} sono rispettivamente la parentesi tonda aperta e chiusa.

Per mezzo di questo linguaggio è possibile generare tutte le formule ben formate della logica proposizionale.
...
In questo caso la proprietà \textit{formula} è derivata mediante l'applicazione della funzione di mapping $m(aer)$ \eqref{function:mapping}, dove $aer$ è la regola di valutazione astratta di partenza, $c_i$ è un controllo, $t_i$ è un test e $e$ è una regola di valutazione concreta.
\begin{equation}
\begin{gathered}
m(aer):
...
\begin{itemize}
    \item{\textit{entrypoint.py}}, il cui ruolo è quello di leggere ed effettuare la deserializzazione degli \textit{input} da \textit{STDIN}, leggere i metadati del driver, e pre-caricare la classe del driver.
    ...
    \item{\textit{driver.py}}, che fornisce un'\textit{interfaccia} mediante la classe \textit{Driver} che ciascun payload dovrà implementare.
        ...
        L'esecuzione effettiva del codice avviene attraverso il metodo \textit{run} e un meccanismo di gestione delle operazioni atomiche in due flussi (forward e rollback) dettagliatamente illustrati nella sezione successiva.
...
\begin{itemize}
    ...
    \item \textit{requirements.txt}, scritto in Python, che contiene tutte le librerie del database PyPI\foontnote{https://pypi.python.org/} da cui il codice del driver dipende.
\end{itemize}
...
\begin{itemize}
    ...
    \item schema, oggetto contenente lo schema JSON utilizzato per effettuare il rendering del form di input nella dashboard
\end{itemize}
...
La proprietà analizzata da questo esempio è la \textit{confidenzialità del dato} ottenuta dall'aggregazione in \texit{and} logico di due diversi controlli:
\begin{enumerate}
