...
È presente un meccanismo di autorizzazione di tipo \textit{RBAC}\footnote{Role-based access control}; non sono applicate politiche sulla lunghezza né sulla complessità delle password; non sono presenti mecccanismi di rotazione delle stesse.

È stato riscontrato l'utilizzo di un server SMTP per l'invio delle e-mail configurato privo di autenticazione e di meccanismi di cifratura dei dati in transito.
...
Execution Manager & L'execution manager riceve i dati in input dai test tramite un meccanismo di comunicazione basato su code. Su questo non sono stati configurati meccanismi di cifratura dei dati, né meccanismi di firma volti a preservare l'integrità dei dati in transito.
I driver dei test sono eseguiti comunicando con un cluster Docker tramite socket Unix in chiaro; il download degli stessi viene fatto dal repository con connessione cifrata tramite SSL e autorizzata con meccanismi \textit{MAC}\footnote{Mandatory Access Control}. 
 \\ \hline
\end{ltabulary}
